...
$row=$res->fetch_array();
		...
		$images=$row[4];
		$con=new MySQLi("localhost","root","","project");
		$sql1="insert into history values(null,$mid,$cid,'$modelno','$modelname',$price,'$images')";
		$res=$con->query($sql1);
		}
?>
<nav>
	<img style="margin:35px;padding-left:150px;padding-top:15px;padding-right:150px;padding-bottom:15px;height:500px" src="<?php echo $row[4] ?>"><br>
	<?php
	    if($row[10]>0)
		{
			if(isset($_COOKIE['user']))
			...
			echo "<td><a href='watchesbuynow.php?model=$row[1]'><button style='background-color:#006600;color:#ffffff;width:260px;height:80px'> BUY NOW </button></a></td></tr></table>";
		}
		else
		{
...
<article>
	...
	<p style="color:#000033;font-size:28px;padding-left:423px"><strike>&#8377;<?php echo $row[8] ?></strike></p>
	<blink style=";padding-left:423px"> OFFER PRICE </blink>
	<p style="color:#000033;font-size:28px;padding-left:423px">&#8377;<?php echo $row[9] ?> </p>
	<br><br>
	...
	<?php
		...
		echo "<tr><td>Series </td><td>" .$row[3]. "</td></tr>";
		echo "<tr><td>Strap Material</td><td>" .$row[5]. "</td></tr>";
		echo "<tr><td>Warranty</td><td>" .$row[6]. "</td></tr></table>";
	?>
</article>
</body>
</html>