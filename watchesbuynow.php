...
if(isset($_SESSION['id']))
{
	...
	$sql="select * from registration,watches where modelno='$model' and cid=$id";
	$res=$con->query($sql);
	$row=$res->fetch_array();
}
else
...
<tr><td>Price : </td><td><input type='text' name='n7' value="<?php echo $row[17] ?>" readonly></td></tr>
...
if(isset($_POST['btn']))
{
	...
	$total=$row[17]*$qty;
	$qty1=$row[18]-$qty;
	$sql1="update watches set qty=$qty1 where modelno='$model'";
	$con->query($sql1);
	$ono=rand(1,99999);
